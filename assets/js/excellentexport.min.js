...
var ExcellentExport = (function() {
    ...
    var ee = {
        /** @export */
        excel: function(anchor, table, name,without) {
            table = get(table);

            var asli = table;
            var d = asli;
            var isi = table.innerHTML;
            if(without!=null){
                var w = document.getElementsByClassName(without)[0];
                if(d.childNodes[1].hasChildNodes(w)){
                   d.childNodes[1].removeChild(w);
                   isi = d.innerHTML;
                   asli.childNodes[1].appendChild(w);
                }
            }

            var ctx = {worksheet: name || 'Worksheet', table: isi};
            var b64 = base64(format(template.excel, ctx));
            return createDownloadLink(anchor, b64, 'application/vnd.ms-excel','export.xls');
        },
        /** @export */
        csv: function(anchor, table, delimiter, newLine,without) {
