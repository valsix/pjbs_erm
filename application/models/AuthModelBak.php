<?php class AuthModel extends _Model{
	...
	public function GetChildJabatan($idjabatan=null, $is_self = true){
		$jabatan = array();
		...
		if($idjabatan){
			...
			$rowschild = $this->conn->GetArray("select id_jabatan 
				...
				and exists (select 1 from public_sys_user b where a.id_jabatan = b.id_jabatan)");
    ...
    public function autoLogin($username=null, $password=null){
		...
		if($rv->RESPONSE == "1"){
			...
			if(strstr(strtolower($_SESSION[SESSION_APP]['nama_group']),"hpe")!==false or strstr(strtolower($_SESSION[SESSION_APP]['nama_group']),"hps")!==false){
				return array(
					'success'=>'login berhasil.',
					'link'=>site_url("panelbackend/hps_formulir")
				);
			}
			else
				return array('success'=>'login berhasil');
