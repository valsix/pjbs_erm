<?php class Risk_risikoModel extends _Model{
	...
	public function getListKertasKerja($param=array())
	{
		...
		if(($wherearr1)){
			...
			from risk_risiko rr
			where rr.is_lock != '0' and ".implode(" and ", $wherearr1)."
		...
		$where = " where rr.is_lock != '0' and 1=1 ";
		if(($wherearr))
		...
		$sql .= $where;
		$sql .= " order by rs.owner, rr.id_risiko, rc.no, rm.no";
	...
	public function _getFilterRisk($id_kajian_risiko=null, $tahun=null, $bulan=null){
		if(!$tahun)
			$tahun = date('Y');
...
*/
	...
	public function getCountAll($id_kajian_risiko=null, $tahun=null, $bulan=null)
	{
		...
		list($where, $where1) = $this->_getFilterRisk($id_kajian_risiko, $tahun, $bulan);

		$sql = "select count(r.id_risiko) as j, t.nama
		from mt_risk_matrix m
		join mt_risk_tingkat t on t.id_tingkat = m.id_tingkat
		left join risk_risiko r on r.inheren_kemungkinan = m.id_kemungkinan and r.inheren_dampak = m.id_dampak /*and r.is_lock != '0' and r.status_risiko = '1'*/ ".$where.$where1."
		...
		left join risk_risiko r on r.control_kemungkinan_penurunan = m.id_kemungkinan and r.control_dampak_penurunan = m.id_dampak /*and r.is_lock != '0' and r.status_risiko = '1'*/ ".$where.$where1."
		group by t.nama,t.id_tingkat
		order by t.id_tingkat desc";
		...
		from mt_risk_tingkat t 
		join mt_risk_matrix m on t.id_tingkat = m.id_tingkat
		left join risk_risiko r on r.residual_target_kemungkinan = m.id_kemungkinan and r.residual_target_dampak = m.id_dampak /*and r.is_lock != '0' and r.status_risiko = '1'*/ ".$where.$where1."
		group by t.nama,t.id_tingkat
		order by t.id_tingkat desc";
		...
		where 1=1 /*r.is_lock != '0' and r.status_risiko = '1'*/ ".$where."
