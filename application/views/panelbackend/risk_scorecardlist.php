<div class="container-fluid">
    ...
    margin-bottom: 0px;">
                        ...
                        <li>
                            <a   href="<?=base_url("panelbackend/risk_scorecard/index/$id_kajian_risiko")?>"><?=strtoupper($mtjeniskajianrisikoarr[$id_kajian_risiko])?></a>
                        </li>

                        <?php foreach($broadcrum as $k=>$v){ ?>
                            <li>
                                <?php if($k){ ?>
                                    <a  href="<?=base_url("panelbackend/risk_scorecard/index/$id_kajian_risiko/$k")?>"><?=$v?></a>
                                <?php }else{ ?>
                                    <?=$v?>
                                <?php } ?>
                            </li>
                    ...
                    <div class="pull-right">
                    	...
                    	<a class="btn btn-success waves-effect" href="<?=site_url('panelbackend/risk_scorecard/index/'.$id_kajian_risiko)?>"><span class="glyphicon glyphicon-backward"></span> BACK</a>
                        <?php }else{ ?>
                        <a class="btn btn-success waves-effect" href="<?=site_url('panelbackend/risk_scorecard')?>"><span class="glyphicon glyphicon-backward"></span> BACK</a>
                        <?php }*/ ?>
                        <?php 
...
<?php $no=1; foreach($rows as $row){ 
    ...
    <tr data-tt-id='<?=$row['id']?>' data-tt-parent-id='<?=$row['id_parent']?>'>
        <td>
            <b><a href="<?=base_url("panelbackend/risk_scorecard/index/$id_kajian_risiko/$row[id_scorecard]")?>"><?=$row['nama']?></a></b>
        </td>
    </tr>
    <?php }elseif($row['owner']==$owner){ ?>
    <tr data-tt-id='<?=$row['id']?>' data-tt-parent-id='<?=$row['id_parent']?>' class='bg-light-blue'>
        <td>
            <a style="color:#fff" href="<?=base_url("panelbackend/risk_risiko/index/$row[id_scorecard]")?>"><?php if($is_proyek){ ?><?=$no++?>. <?php } ?> <?=$row['nama']?></a>
        </td>
        <?php if($is_proyek){ ?>
    ...
    <tr data-tt-id='<?=$row['id']?>' data-tt-parent-id='<?=$row['id_parent']?>'>
        <td>
            <a href="<?=base_url("panelbackend/risk_scorecard/edit/$id_kajian_risiko/$row[id_scorecard]")?>"><?=$row['nama']?></a>
        </td>
    </tr>
    <?php }else{ ?>
	<tr data-tt-id='<?=$row['id']?>' data-tt-parent-id='<?=$row['id_parent']?>'>
		<td>
			<a href="<?=base_url("panelbackend/risk_risiko/index/$row[id_scorecard]")?>"><?php if($is_proyek){ ?><?=$no++?>. <?php } ?><?=$row['nama']?></a>
		</td>
        <?php if($is_proyek){ ?>
