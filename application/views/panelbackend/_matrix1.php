<div id="divmatrix" style="background-color:#fff;text-align: left; width:auto; max-width:370px; margin: auto;">
...
?>
    <table class="tbmatrix">
        ...
        ?>
<a href="" id="downloadmatrix" download="matrix.jpeg" class="btn btn-sm btn-success"><span class="glyphicon glyphicon-floppy-save"></span></a>
        <?php
        echo "</td>";
        foreach($rs_dampak as $r_d => $val_d){
        ...
        <tr>
          ...
          <td rowspan="4" align="right">
            <?php if(!$_SESSION[SESSION_APP]['group_id']==1){ ?>
              <button style='font-size: 10px; padding:5px; font-weight: bold;'type='button' class='btn btn-warning' data-toggle="modal" data-target="#tablematrix<?=$id_class?>" onclick="reqtb(<?=$id_class?>, <?=$id_kajian_risiko?>)"><i class="material-icons" style="font-size: 24px">table_chart</i><br/>DAFTAR RISIKO</a></button>
            <?php } ?>
        </tr>
      <?php if($rating['i']){ ?>
        <tr>
...
<script type="text/javascript">
  $(function(){
    html2canvas($("#divmatrix")).then(canvas => {

        var image = canvas.toDataURL("image/jpeg").replace("image/jpeg", "image/octet-stream");  

        $("#downloadmatrix").attr("href",image);

        // $("#divmatrix").html(canvas);
