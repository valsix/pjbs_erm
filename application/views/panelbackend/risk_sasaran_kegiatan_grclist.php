...
if($rowheader['jenis_sasaran']=='3'){ 
  ...
  if($rowheader1['id_sasaran_kegiatan'] && $rowheader1['id_sasaran_kegiatan'] && ($rowheader1['id_kpi']) && $edited){
    $from = '<a class="btn waves-effect btn-lg btn-success" href="'.base_url("panelbackend/risk_risiko_grc/add/".$rowheader['id_scorecard']."/".$rowheader1['id_sasaran_strategis']."/".$rowheader1['id_sasaran_kegiatan'])."/".implode("-",$rowheader1['id_kpi']).'">NEXT NEW '.strtoupper($label_risk).' <span class="glyphicon glyphicon-forward"></span></a> ';
...
if($rowheader1['id_sasaran_strategis'] && ($rowheader1['id_kpi']) && $edited){
  $from = '<a class="btn waves-effect btn-lg btn-success" href="'.base_url("panelbackend/risk_risiko_grc/add/".$rowheader['id_scorecard']."/".$rowheader1['id_sasaran_strategis'])."/0/".implode("-",$rowheader1['id_kpi']).'">NEXT NEW '.strtoupper($label_risk).' <span class="glyphicon glyphicon-forward"></span></a> ';
