...
class Risk_control extends _adminController{
	...
	protected function init(){
		...
		elseif ($this->mode == 'detail'){
			...
			$this->data['page_title'] = 'Daftar Kontrol';
			$this->data['edited'] = true;
		}
	...
	protected function Rules(){
		...
		if($this->data['edited'] && $this->data['row']['is_lock'] && !$this->access_role_custom['panelbackend/risk_risiko']['view_all_direktorat'])
			$return = true;
	...
	public function Edit($id_risiko=null, $id=null){
		...
		if ($this->post['act'] === 'save' or $this->post['act']=='save_rekomendasi' or $this->post['act']=='save_review') {
			...
			if($this->post['act'] === 'save'){
				...
				if($this->access_role_custom['panelbackend/risk_risiko']['view_all_direktorat']){
					$record['status_konfirmasi'] = 1;
					$record['is_lock'] = 1;
				}
			}
	...
	private function _notifRekomReview($id_risiko=null,$id=null){
		$record = array(
			...
			'url'=>"panelbackend/risk_control/edit/$id_risiko/$id"
		);
	...
	private function _kirimRekom($id_risiko=null,$id=null){
		...
		$record = array(
			...
			'url'=>"panelbackend/risk_control/detail/$id_risiko/$id"
		);
	...
	private function _verifiedRekom($id_risiko=null,$id=null){
		...
		$record = array(
			...
			'id_risiko'=>$id_risiko,
			'untuk'=>$this->data['rowheader']['owner'],
			'url'=>"panelbackend/risk_control/detail/$id_control/$id"
		);
		$this->InsertTask($record);
	...
	protected function _afterDetail($id){
		...
		foreach($this->data['mtefektifitasarr'] as $r){
			if($r['need_lampiran']){
				...
				and periode = ".$this->conn->escape($periode));
				...
				if(!$this->data['row']['file'][$r['id_efektifitas']][$periode])
					$this->data['row']['file'][$r['id_efektifitas']][$periode] = $rsfile;
			}
		}
	...
	protected function _beforeEdit(&$record=array(), $id){
		$this->_validAccessTask('panelbackend/risk_risiko',$this->data['rowheader1'],$this->data['edited']);
