...
class Mt_sdm_jabatan extends _adminController{
	...
	protected function Rules(){
		$id_struktur = $this->data['row']['id_struktur'];
		if($id_struktur)
			$this->data['mtsdmstrukturarr'][$id_struktur] = $this->conn->GetOne("select nama from mt_sdm_struktur where id_struktur = ".$this->conn->escape($id_struktur));
	...
	public function Index($page=0){
		...
		if($_SESSION[SESSION_APP]['tgl_efektif']){
			...
			$this->_setFilter(" '$tgl_efektif' between nvl(a.tgl_mulai_efektif, '$tgl_efektif')and nvl(a.tgl_akhir_efektif,'$tgl_efektif') ");
