...
class FileStation extends Authenticate
{
    ...
    public function upload($sessionId, $folder_path, $file, $renameFile="", $force_parent = true, $overwrite = true, $additional = false)
    {

        $nameFile = ($renameFile == "") ? basename($file) : $renameFile;
        /* kasih rename disini */
        $cfile1 = new \CURLFile(
            realpath($file),
        ...
        return $this->_post($sessionId, 'Upload', 'entry.cgi', 'upload', [
            'path'              => $folder_path,
            'file'              => $cfile1,
            'create_parents'    => $force_parent,
            'overwrite'         => $overwrite
        ]);
    }
